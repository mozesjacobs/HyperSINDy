...
def parse_hyperparams():
    ...
    parser.add_argument('-B', '--beta', default=25.0, type=float, help="KL divergence weight in loss (only for HyperSINDy)")
    parser.add_argument('-WD', '--weight_decay', default=1e-2, type=float, help="Weight decay for sindy coefficients (only for SINDy)")
...
def parse_args():
    ...
    parser.add_argument('-TB', '--tensorboard_folder', default='./runs/drift_diff/', type=str, help="Output folder for tensorboard")

    # saving specifics
    parser.add_argument('-sess', '--session_name', default='1', type=str, help="Appended to last part of file names")
    parser.add_argument('-DAT', '--date', default="12-02-22", type=str, help="The date"),
