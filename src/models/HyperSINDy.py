...
class Net(nn.Module):
    def __init__(self, args, hyperparams):
        ...
        self.hypernet = HyperNet(self.noise_dim, (self.z_dim, self.z_dim),
            [self.hypernet_hidden_dim for _ in range(4)])

        self.sindy_coeffs = nn.Parameter(torch.ones(self.library_dim, self.z_dim),
            requires_grad=True)
    ...
    def forward(self, x, x_lib=None, device=0):
        ...
        # drift
        drift = torch.matmul(x_lib, self.get_masked_coefficients())

        # diff: batch x z
        diff_term = self.sample_diffusion(batch_size=x.size(0), device=device)
    ...
    def sample_diffusion(self, n=None, batch_size=None, device=0):
    ...
    def update_threshold_mask(self, threshold, device):
        self.threshold_mask[torch.abs(self.sindy_coeffs) < threshold] = 0
